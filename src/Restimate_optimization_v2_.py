...
combining 
	...
	optimization for 1, 2, 3 color/pix
'''
...
#	print ''; print ''
	...
	if option_run == '1colorperpix':
		Aarr_ = (np.random.uniform(2e-5,A_tot/4.,5))  # 5mm diameter circle has a diameter of 2e-5 m^2
		if (A_tot - np.sum(Aarr_)) <= 0: continue
		Aarr = np.hstack((Aarr_, A_tot - np.sum(Aarr_)))
		dpix_arr = (np.random.uniform(2.,25.,6))*1.e-3
		Npix_arr = np.int_(2./np.sqrt(3.) * Aarr/dpix_arr**2.)
		ind = np.where(Npix_arr == 0)
		...
		#		Npix_arr[j] = np.float_(out[9,ind[0]])
			...
			uKarcmin_arr[j] = 10800./pi*np.sqrt(8.*pi*NET_to_w_arr[j]**2/(2.*Npix_arr[j]*obs_time_sec))/margin
		A_fp_arr_ = np.sqrt(3.)/2. * np.sum(Npix_arr*dpix_arr**2)

	if option_run == '2colorperpix':
		Aarr_ = (np.random.uniform(2e-5,A_tot/1.5,2))  # 5mm diameter circle has a diameter of 2e-5 m^2
		if (A_tot - np.sum(Aarr_)) <= 0: continue
		Aarr = np.hstack((Aarr_, A_tot - np.sum(Aarr_)))
		dpix_arr_ = (np.random.uniform(2.,25.,3))*1.e-3
		Npix_arr_ = np.int_(2./np.sqrt(3.) * Aarr/dpix_arr_**2.)
		ind = np.where(Npix_arr_ == 0)
		if len(ind[0]) > 0: continue
		...
		#		Npix_arr[j] = np.float_(out[9,ind[0]])
			NET_to_w_arr[j] = out[32,ind[0]]
			uKarcmin_arr[j] = 10800./pi*np.sqrt(8.*pi*NET_to_w_arr[j]**2/(2.*Npix_arr[j]*obs_time_sec))/margin
		A_fp_arr_ = np.sqrt(3.)/2. * np.sum(Npix_arr_*dpix_arr_**2)

	if option_run == '3colorperpix':
		Aarr_ = (np.random.uniform(2e-5,A_tot,1))  # 5mm diameter circle has a diameter of 2e-5 m^2
		if (A_tot - np.sum(Aarr_)) <= 0: continue
		Aarr = np.hstack((Aarr_, A_tot - np.sum(Aarr_)))
	...
	if option_run == '6colorperpix':
#		Aarr_ = (np.random.uniform(2e-5,A_tot,1))  # 5mm diameter circle has a diameter of 2e-5 m^2
#		if (A_tot - np.sum(Aarr_)) <= 0: continue
#		Aarr = np.hstack((Aarr_, A_tot - np.sum(Aarr_)))
		dpix_arr_ = (np.random.uniform(2.,25.,1))*1.e-3
		Npix_arr_ = np.int_(2./np.sqrt(3.) * A_tot/dpix_arr_**2.)
		ind = np.where(Npix_arr_ == 0)
		if len(ind[0]) > 0: continue
		dpix_arr = np.array([dpix_arr_[0],dpix_arr_[0],dpix_arr_[0],dpix_arr_[0],dpix_arr_[0],dpix_arr_[0]])
		Npix_arr = np.ones(6)*Npix_arr_
		for j in range(num_band):
			if j==0: out = out1
			if j==1: out = out2
			if j==2: out = out3
	...
	Ndet_sum_ = np.sum(Npix_arr)*2
	if ((Ndet_sum_ > Ndet_tot_max) | (Ndet_sum_ < Ndet_tot_min)): continue #print 'out count', Npix_sum_; continue
...
#	sys.exit()
	...
	if ((r_wm_i[1]/np.sqrt(fsky)) > 0.001): continue #print 'out error', (r_wm_i[1]/np.sqrt(fsky)); continue

	A_fp_arr.append( A_fp_arr_ )
	Npix_sum.append(Npix_sum_)
	Ndet_sum.append(Ndet_sum_)
	...
	np.savez(filename_out+'.npz', \
		...
		r_in=r_in, \
		A_tot=A_tot, \
		Ndet_tot_max=Ndet_tot_max, \
		nu_arr=nu_arr, 
		num_monte=num_monte, \
		obs_time_sec=obs_time_sec, \
		help='uKarcmin1~6_arr, Npix1~6_arr, dpix1~6_arr, A_fp_arr, Npix_sum, Ndet_sum, uKarcmin_tot, r_wm_i_arr_m, r_wm_i_arr_s, fsky, r_in, A_tot, Ndet_tot_max, nu_arr, num_monte, obs_time_sec, monte'
		)
...
np.savez(filename_out+'.npz', \
	...
	monte=monte, \
	obs_time_sec=obs_time_sec, \
	help='uKarcmin1~6_arr, Npix1~6_arr, dpix1~6_arr, A_fp_arr, Npix_sum, uKarcmin_tot, r_wm_i_arr_m, r_wm_i_arr_s, fsky, r_in, A_tot, Ndet_tot_max, nu_arr, num_monte, obs_time_sec, monte'
	)
...
width = 0.35  
p1 = py.bar(monte, Npix1_arr*2, width, color='r')

Npix_accum = np.array(Npix1_arr*2)
p2 = py.bar(monte, Npix2_arr*2, width, color='g', bottom=Npix_accum)

Npix_accum = np.array(Npix2_arr*2) + Npix_accum
p3 = py.bar(monte, Npix3_arr*2, width, color='b', bottom=Npix_accum)

Npix_accum = np.array(Npix3_arr*2) + Npix_accum
p4 = py.bar(monte, Npix4_arr*2, width, color='c', bottom=Npix_accum)

Npix_accum = np.array(Npix4_arr*2) + Npix_accum
p5 = py.bar(monte, Npix5_arr*2, width, color='m', bottom=Npix_accum)

Npix_accum = np.array(Npix5_arr*2) + Npix_accum
p6 = py.bar(monte, Npix6_arr*2, width, color='y', bottom=Npix_accum)
